...
class LTISystem:
    ...
    def output_super(self, super_sig: SuperSignal):
        res = None
        for coeff, sig in super_sig.components:
            comp = self.output(sig)
            scaled_comp = comp.multiply(coeff)

            if res is None:
                res = scaled_comp
            else:
                res = res.add(scaled_comp)
        return res
...
if __name__ == "__main__":
    ...
    # Component signals
    x1 = DiscreteSignal(0, INF)
    x1.set_value_at_time(0, 1)

    x2 = DiscreteSignal(0, INF)
    ...
    x_comb = x1.multiply(2).add(x2.multiply(-1))
    superposition_out = system.output_by_superposition(x_comb)

    super_signal_out = system.output_super(super_signal)

    print("Outputs are equal?- ", np.allclose(superposition_out.values,super_signal_out.values))