...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    while len(text) > 0:
        chunk = text[-base_digits:]
        limbs.append(int(chunk))
        text = text[:-base_digits]

    res = np.array(limbs, np.int64)
    return sign, res
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    for limb in limbs:
        ...
        chunk_str = f"{res:0{base_digits}d}"
        res_strings.append(chunk_str)

    if carry > 0:
        res_strings.append(str(carry))


    final_str = "".join(reversed(res_strings))
    final_str = final_str.lstrip('0')

    if not final_str: return "0"
    if(sign == -1): final_str = "-" + final_str
    return final_str
...
def multiply_transform(a, b, engine):
    ...
    min_len = len(a) + len(b) -1
    N = next_power_of_two(min_len)
    #np.pad -> (arr to pad, (left padding, right padding)) default zero
...
def multiply_schoolbook(a, b):
    ...
    a_len = len(a)
    b_len = len(b)
    result = np.zeros(a_len + b_len -1, dtype= np.int64)
...
def multiply(text_a, text_b, method):
    ...
    if method.lower() == "fft":
        ...
        engine = DFTAnalyzer()
    elif method.lower() == "arbitrary":
        engine = ArbitraryLengthFFT()
        
    a_sign, a_limb = to_limbs(text_a)
    b_sign, b_limb = to_limbs(text_b)

    if method.lower() == "schoolbook":
        raw_conv_arr = multiply_schoolbook(a_limb, b_limb)
        N = 0 
    else:
        raw_conv_arr, N = multiply_transform(a_limb, b_limb, engine)

    prod_str = from_limbs(a_sign*b_sign, raw_conv_arr)

    return (prod_str, N, a_limb, b_limb)
...
def run_single(path, method, out_dir):
    ...
    a_digits = len(text_a.lstrip('-+'))
    b_digits = len(text_b.lstrip('-+'))
    prod_str,N, limbs_a, limbs_b = multiply(text_a,text_b,method)

    expected = int(text_a) * int(text_b)
    verdict = ""
    if(int(prod_str) == expected):
        verdict = "MATCH"
    else: verdict = "MISMATCH"

    print(f"Integer Multiplication Verdict: {verdict}")

    save_path_prod = os.path.join(out_dir, "product.txt")
    write_text(save_path_prod,prod_str)

    report_lines = [
        "Task A -- big-integer multiplication by spectral convolution",
        f"input file          : {path}",
        f"method              : {method}",
        f"digits of A / B     : {a_digits} / {b_digits}",
        f"base                : 10^{BASE_DIGITS}",
        f"limbs of A / B      : {len(limbs_a)} / {len(limbs_b)}",
        f"transform length N  : {N}",
        f"digits of product   : {len(prod_str.lstrip('-'))}",
