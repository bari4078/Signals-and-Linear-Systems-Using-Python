...
def convolve_plane_direct(plane, kernel):
    ...
    return out
def deconvolve_plane(plane, kernel, engine, circular=False, eps=1e-3):
    """
    Undo convolve_plane: recover an estimate of the original plane from a
    blurred plane, by inverse filtering in the frequency domain.

    This mirrors convolve_plane's padding/cropping exactly, but divides by
    the kernel's spectrum instead of multiplying. A pure inverse filter
    (1 / Kernel) is unusable wherever Kernel is near zero (or wherever the
    blurred input carries noise), so this uses a regularized inverse:

        Kernel_inv = conj(Kernel) / (|Kernel|^2 + eps)

    which is the standard Wiener-filter form with a flat noise/signal
    estimate. eps -> 0 recovers the naive inverse filter; larger eps trades
    sharpness for stability.

    Parameters
    ----------
    plane : 2D numpy array of float -- the blurred plane, SAME SIZE as the
            original (i.e. what convolve_plane returns), values in [0, 1]
    kernel : 2D numpy array of float, sums to 1 -- the blur kernel used
    engine : DFTAnalyzer or FFTTransformer
    circular : bool -- must match how the blur was produced
    eps : float -- Tikhonov/Wiener regularization constant
    ...
    if not circular:
        ...
        # convolve_plane cropped its padded result to
        # [row_start:row_start+H, col_start:col_start+W]; to invert we have
        # to re-embed the blurred plane at that same offset before
        # transforming, or the spectra won't correspond to the same kernel.
        plane_padded = np.zeros((pad_H, pad_W))
        plane_padded[row_start:row_start + H, col_start:col_start + W] = plane

        kernel_padded = np.pad(kernel, (
            (0, pad_H - kh), (0, pad_W - kw)
        ))

        Plane = transform_2d(plane_padded, engine)
        Kernel = transform_2d(kernel_padded, engine)

        Kernel_inv = np.conj(Kernel) / (np.abs(Kernel) ** 2 + eps)
        Restored = inverse_2d(Plane * Kernel_inv, engine)
        Restored = np.real(Restored)

        return Restored[row_start:row_start + H, col_start:col_start + W]
        ...
        Plane = transform_2d(plane, engine)
        Kernel = transform_2d(kernel_rolled, engine)
...
def deconvolve_image(image, kernel, engine, circular=False, eps=1e-3):
    """
    Apply deconvolve_plane to a whole image (grayscale or colour), exactly
    the way convolve_image applies convolve_plane.
    """
    if image.ndim == 2:
        return deconvolve_plane(image, kernel, engine, circular, eps)

    result = np.zeros_like(image)
    for i in range(3):
        result[:, :, i] = deconvolve_plane(image[:, :, i], kernel, engine, circular, eps)
    return result
